...
class XEDATheory:
    def __init__(self, numcores=1, printlevel=2, label='xeda', 
        ...
        if not eda and (eda_atm is not None or eda_charge is not None or eda_mult is not None):
            raise ValueError("You have set eda to False but provided eda_atm, eda_charge or eda_mult. Please set eda to True or remove these keywords.")
        if eda:
            if eda_atm is None or eda_charge is None or eda_mult is None: 
                raise ValueError("You have set eda to True but did not provide eda_atm, eda_charge or eda_mult. Please set eda to False or provide these keywords.")  
            if not len(eda_atm) == len(eda_charge) == len(eda_mult):
                raise ValueError("You have provided eda_atm, eda_charge and eda_mult but they are not the same length. Please check.")
    ...
    def set_embedding_options(self, PC=False, MM_coords=None, MMcharges=None):
        if PC is True:
            ...
            self.bc = builder.charge_info(self.mol, np.ravel(np.column_stack((MMcharges[:, np.newaxis], MM_coords / ash.constants.bohr2ang))))    
    ...
    def create_mol(self, qm_elems, current_coords, charge, mult):
        ...
        import py_api.mole as mole
        if not self.eda:
            coords_string=ash.modules.module_coords.create_coords_string_xscf(qm_elems,current_coords)
            self.mol = mole.mol_info(f'{charge} {mult};\n' + coords_string, bas_str=self.basis)
        else:
            coords_string=ash.modules.module_coords.create_coords_string_xeda(qm_elems, current_coords
                                                                              , self.eda_atm, self.eda_charge, self.eda_mult)
    ...
    def run_SCF(self):
        ...
        if self.scf_type == 'RHF' or self.scf_type == 'RKS':
            ...
            raise NotImplementedError("ROHF/ROKS functionality is not yet implemented.")
    ...
    def run_EDA(self):
        ...
        if self.ct is not True:
            ...
            raise NotImplementedError("Charge-transfer functionality is not yet implemented.")
